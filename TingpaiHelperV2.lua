...
local function checkshisan1(tab, laiziNum)
    local has_single = false
    local ting_tab1 = {}
    local ting_tab2 = {}
    local temp_dui = {}
    local tab_temp = {11,19,21,29,31,39,41,42,43,44,45,46,47} --记录每张都不一样的情况
    local tab_temp1 = {11,19,21,29,31,39,41,42,43,44,45,46,47} --记录有一对
    local clone_temp = {11,19,21,29,31,39,41,42,43,44,45,46,47}  --记录有一对
    local countdui = false 
    local count1 = 0
    local count = 0
    --没有对的情况下
    for i , k in pairs(tab)do
        local index = table.indexof(tab_temp,i)
        if index then
           tab_temp[index] = 0
           count = count + 1  
        end 
    end
    for i , k in pairs(tab_temp1) do
        ting_tab1[k] = true
    end 
    if count + laiziNum == 13 then 
       return true ,ting_tab1 -- 听十三张牌
    end 
   

    --有对的情况下 
    for i , k in pairs(tab) do
        local index = table.indexof(tab_temp1,i) --第一个index
        if index then
            if not temp_dui[index] or (temp_dui[index] and not countdui) then
               temp_dui[index] = (temp_dui[index] or 0)+ 1 --第一对
               clone_temp[index] = 0 --设置已经遍历过
            end 
            if temp_dui[index] == 2 then 
               countdui = true --有对子
            end 
        end 
    end 
    ...
    local select_map = {}
    for key ,value in pairs(clone_temp) do 
        if value ~= 0 then 
           table.insert(select_map,value)
           ting_tab2[value] = true
        end  
    end 

    local length = #select_map 
    if length - laiziNum == 1 then 
        return true ,ting_tab2 -- 对子牌以下的
    end 
...
function getAllting(cards, config, laizi)
    ...
    cards = clone(cards)
    table.sort(cards) 
    local joinKey = StringUtil.join(cards,",")
    -- trace("joinKey 。。 "  .. joinKey) 
    if catcheData[joinKey] then 
       -- trace("取得缓存数据 。。 "  .. joinKey) 
       return catcheData[joinKey]
    end 
    local tingArr = {}
    local is_ting = false
    if config then
        local stackCards = tranCardsTab(cards)
        local stackCards1 = clone(stackCards)
        if #cards == 13 then
            if config.qidui1 then
                is_ting, tingArr = checkqidui(stackCards, 0)
            end
            if config.qidui2 then
                local laizi_num = stackCards[laizi] or 0
                stackCards[laizi] = nil
                is_ting, tingArr = checkqidui(stackCards, laizi_num)
            end
            if config.shisan1 then 
               --十三幺
               local laizi_num = stackCards1[laizi] or 0
               stackCards1[laizi] = nil
               is_ting, tingArr = checkshisan1(stackCards1, laizi_num)
            end 
        end
    end
    --print("dddddddddddasdfgsagfhsagda---->>")
    --print_r(cards)
    if is_ting then
        local tingArr1 = getTingNumArr(cards,laizi)
        tingArr = table2Table(tingArr,tingArr1)
        catcheData[joinKey] = tingArr
        return  tingArr --v2ToV1Data(tingArr)
    else
        tingArr = getTingNumArr(cards,laizi)
        catcheData[joinKey] = tingArr
        return tingArr --v2ToV1Data(tingArr)
    end
    catcheData[joinKey] = tingArr
...
function table2Table (array1 , array2) 
    local arrayOne = clone(array1)
    local arrayTwo = clone(array2)
    for key ,value in pairs (arrayOne) do
        arrayTwo[key] = true
    end
